...
public class DestructionSystem : ComponentSystem
{
    ...
    protected override void OnUpdate()
    {
        ...
        //loop through everything with the tag to be able to destroy it
        Entities.WithAll<PickupTag>().ForEach((Entity upgrade, ref Translation upgradePos) =>
        {
            playerPos.y = upgradePos.Value.y;

            //if player and enemy overlap
            if (math.distance(upgradePos.Value, playerPos) <= thresholdDis)
            {
                //destroy the entity
                PostUpdateCommands.DestroyEntity(upgrade);
                GameManager.UpgradePlayer();
            }
        });
    }
}
